...
class TestAPI:
    ...
    async def test_get_price_stats_not_found(self, client: AsyncClient):
        """Test statistics for non-existent data"""
        response = await client.get("/api/v1/prices/stats?ticker=btc_usd&days=7")
        assert response.status_code == 404